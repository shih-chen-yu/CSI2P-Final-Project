...
namespace FontSetting {
	...
	const char NotoSansCJK_font_path[] = "./assets/font/NotoSansTC-Regular.ttf";
}
...
FontCenter::init() {
	for(const int &fs : FontSize::list) {
		...
		NotoSansCJK[fs] = al_load_ttf_font(FontSetting::NotoSansCJK_font_path, fs, 0);
	}
}
...
FontCenter::~FontCenter() {
	...
	for(auto &[size, font] : courier_new)
		al_destroy_font(font);
	for(auto &[size, font] : NotoSansCJK)
		al_destroy_font(font);
}
